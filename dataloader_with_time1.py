...
def loadData(file_path, start_time, end_time):
    ...
    for row in range(len(lines)):
        if (row == 0):
            ...
            vza_list = []
            allData_Index = []
            key, pointLngLat = processPixelTimeSeries(lines[row].strip(), time_list, normalized_ntl_list, vza_list, allData_Index, start_time, end_time)
            key_list.append(key)
            lng_lat_list.append(pointLngLat)
            ...
            datasets.append([time_list, normalized_ntl_list, allData_Index, all_crossValidation_trianingData_Index, all_crossValidation_validationData_Index, lng_lat_list, key_list])
        print(row, "line read...")
...
if __name__ == "__main__":
    ...
    file_path = "G:\\postgraduate\\postgraduate_bishe\\TS_Txt_Constrained_By_LC_Larger50%_3_3\\TS_output_withoutMoonIlluminationAndSnow_fit_result_gradientDescent_1%_1invalid_Antakya"
