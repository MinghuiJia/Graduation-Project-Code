...
def normalizationZenith(visX_dic,visY_dic,outputFile,output_params_path):
    ...
    for key in visX_dic:
        ...
        parameters1 = np.polyfit(visX_dic[key], ccc, 1)
        x1 = np.arange(0, 90, 0.02)
        y1 = parameters1[0] * x ** 1 + parameters1[1]


        # plt.scatter(visX_dic[key], ccc, color='green', s=10, label='correct points')
        plt.scatter(visX_dic[key], ccc, color='green', s=10, label='角度校正后数据')
        # plt.plot(x, y, color='red', label='quadratic fit')
        plt.plot(x, y, color='red', label='二次拟合曲线')
        plt.plot(x1, y1, color='red', label='线性拟合曲线')
        ...
        plt.xlabel(u"传感器观测天顶角", fontproperties=font_manager.FontProperties(fname="C:/Windows/Fonts/simhei.ttf",weight='extra bold'),fontsize=22)
        # plt.ylabel(r"$\bf{DNB\ BRDF-Corrected\ NTL\ Radiance}$",{'size': 16})
        plt.ylabel(r"夜光辐亮度", labelpad=5,fontproperties=font_manager.FontProperties(fname="C:/Windows/Fonts/simhei.ttf",weight='extra bold'),fontsize=22)
...
if __name__=="__main__":
    ...
    for name in name_list:
        work_dic = "D:\\VZA_article\\data\\event_longTime\\id18_20180121_20190120\\TS_Txt_Constrained_By_LC_Larger50%\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_"+str(name)
        fit_result_path = "D:\\VZA_article\\data\\event_longTime\\id18_20180121_20190120\\new_method\\TS_output_withoutMoonIlluminationAndSnow_fit_result_gradientDescent_1%_1invalid_111"+str(name)
        output_dic = "D:\\VZA_article\\data\\event_longTime\\id18_20180121_20190120\\new_method\\TS_output_withoutMoonIlluminationAndSnow_visFitResult_gradientDescent_1%_1invalid_111"+str(name)
        output_params_path = "D:\\VZA_article\\data\\event_longTime\\id18_20180121_20190120\\new_method\\TS_output_withoutMoonIlluminationAndSnow_fitParams_gradientDescent_1%_1invalid_111"+str(name)
