...
def readFileAfterDataFilling2(filePath):
    f = open(filePath,'r')
    lines = f.readlines()
    pointsDic = {}
    for i in range(len(lines)):
...
def visDataFillingTimeSeries(pointsDic,pointsDic_fit, pointsLoc, area, show_point_key, start_time, end_time):
    ...
    for key in pointsDic_fit:
        pointsDic_less10_fit[key] = []
        for item in pointsDic_fit[key]:
            item_temp = [item[0], item[1], item[2]]
            pointsDic_less10_fit[key].append(item_temp)
        pointsDic_less10_fit[key].sort(key=lambda x: x[2])  # 指定某列排序
...
if __name__ == "__main__":
    ...
    fit_result_path = "D:\\VZA_article\\data\\event_shortTime\\id18_20170901_20180401\\new_method\\TS_output_withoutMoonIlluminationAndSnow_fit_result_gradientDescent_1%_1invalid_" + city
    datafilling_afterfit_result_path = "D:\\VZA_article\\data\\event_shortTime\\id18_20170901_20180401\\new_method\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_" + city + "_more50_wholeSeries_Prophet_afterfit"
    ...
    visVZATimeSeries(pointsDic, pointsDic_fit, city, point, start_time, end_time)

    # # 可视化时间序列，并计算NDHDNTL
    visTimeSeries(pointsDic, pointsDic_fit, city, point, start_time, end_time)
    # 可视化角度校正补值前后的对比图
    visDataFillingTimeSeries(pointsDic_fit, pointsDic_datafilling_afterfit, pointsDic, city, point, start_time, end_time)
