...
def fit_prophet(time_series, NTLValue_series, alldataIndex, trainingIndex, validationIndex, PSO_Params, pic_save_dir, key, j):
    ...
    for i in range(len(time_series)):
        if (i in validationIndex):
            ...
            datetime_temp = dtime.datetime.strptime(str(time_series[i]), "%Y%m%d")
            validation_Time.append(datetime_temp)
    ...
    if (len(PSO_Params) != 0):
        ...
        particle_num = PSO_Params[0]
        iter_num = PSO_Params[1]
        min_value = PSO_Params[2]
        c1 = PSO_Params[3]
        c2 = PSO_Params[4]
        w = PSO_Params[5]
    ...
    #可视化结果
    m.plot(forecast)  # 将预测的效果进行绘图
    # 生成测试点的ori和pred数据
    test_ori = validation_NTLValues
    test_pred = []
    test_time = []
    for eachIndex in validationIndex:
        pred = forecast.loc[eachIndex, 'yhat']
        test_pred.append(pred)
        test_time.append(forecast.loc[eachIndex, 'ds'])

    plt.scatter(test_time, test_ori, color="red", label="ORI", s=25)
    plt.scatter(test_time, test_pred, color='green', label="predicted point", s=15)
    # 绘制竖线
    for x_value in validation_Time:
        plt.axvline(x=x_value, color='lightgrey', linewidth=1, linestyle='--')
    ...
    plt.savefig(pic_save_dir + "\\" + key + "_" + str(j) + ".png", bbox_inches='tight')
...
if __name__=="__main__":
    ...
    # 这是角度校正前的数据对应的文件夹
    time_series_type = "represent_pixel_datafilling"
    ...
    for name in name_list:
        ...
        pic_save_dir = parent_dir + time_series_type + "\\" + name
        ...
        for i in range(len(datasets)):
            ...
            for j in range(cross_validation_count):
                result = pool.apply_async(func=fit_prophet,
                                          args=(eachPixel[0],
                                                eachPixel[1],
                                                eachPixel[2],
                                                eachPixel[3][j],
                ...
                # fit_prophet(eachPixel[0],eachPixel[1],eachPixel[2],eachPixel[3][j],eachPixel[4][j],PSO_Params,pic_save_dir,key_list[i],j)
