...
def draw_boxplot(num1, num2, num3, num4, num5, num6,
    ...
    df = pd.concat([df1, df2, df3, df4, df5, df6,
                    ...
                    df13, df14, df15, df16, df17, df18, df19, df20], axis=0)
    ...
    # plt.legend(loc='upper right', prop={'size': 12, "family": "Adobe Gothic Std"})
    sns.move_legend(ax, bbox_to_anchor=(-0.05, -0.95), loc='upper left')
    plt.show()

# 四个时期的对比结果 灾前1个月 灾后10天 灾后1个月 灾后3个月
if __name__=="__main__":
    ...
    temp_post3month_list = []
    for city in country_list:
        dataFile = work_dir + city + "_post_3month_disaster_Img.tif"
        data_DNB, geoTransform_DNB, proj_DNB = read_tiff(dataFile)
        data_DNB_2dim = data_DNB[:, :, 0]
        data_DNB_1dim = data_DNB_2dim.flatten()
        data_DNB_without_nan = data_DNB_1dim[~np.isnan(data_DNB_1dim)]
        temp_post3month_list.append(data_DNB_without_nan / 10.0)


    draw_boxplot(
        temp_pre_list[0], temp_post10day_list[0], temp_post1month_list[0], temp_post3month_list[0],
        temp_pre_list[1], temp_post10day_list[1], temp_post1month_list[1], temp_post3month_list[1],
        temp_pre_list[2], temp_post10day_list[2], temp_post1month_list[2], temp_post3month_list[2],
        temp_pre_list[3], temp_post10day_list[3], temp_post1month_list[3], temp_post3month_list[3],
        temp_pre_list[4], temp_post10day_list[4], temp_post1month_list[4], temp_post3month_list[4],
    )