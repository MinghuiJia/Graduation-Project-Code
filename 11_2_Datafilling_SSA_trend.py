...
if __name__=="__main__":
    ...
    for name in name_list:
        ...
        for i in range(len(datasets)):
            ...
            result = fit_SSA_getTS(eachPixel[0], eachPixel[1], eachPixel[2], key_list[i], False, pic_save_dir)
            results.append(result)

        # 结果写入文件
        outputFile = parent_dir + time_series_type + "\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_" + str(name) + "_more50_wholeSeries_SSA"
        f = open(outputFile, 'w')
        time_list = []
        if (len(datasets)):
