...
def fit_prophet(time_series, NTLValue_series, alldataIndex, trainingIndex, validationIndex, PSO_Params, key):
    column_name = ['ds', 'y']
...
if __name__=="__main__":
    ...
    for name in name_list:
        ...
        # 结果写入文件
        outputFile = parent_dir + time_series_type + "\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_"+str(name)+"_more50_Prophet_accuracy_error"
        f = open(outputFile, 'w')
        key_list = []
        if (len(datasets)):
