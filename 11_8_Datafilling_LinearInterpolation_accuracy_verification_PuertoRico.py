...
def mean_forecast_err1(test_NTLValue_series,testIndex_list,df_dsAndyhat):
    ...
    for tsIndex in testIndex_list:
        ...
        count_index += 1
        relative_error = math.fabs(y-yhat) / y
        mean_value += relative_error
    mean_value = mean_value / len(testIndex_list)
    return mean_value

def fit_LinearInterpolation(time_series, NTLValue_series, alldataIndex, trainingIndex, validationIndex, key):
    ...
    relative_error_once = mean_forecast_err1(validation_NTLValues, validationIndex, pred)
...
def fit_LinearInterpolation_getTS(time_series, NTLValue_series, alldataIndex, key, flag, pic_save_dir):
    # 重新组织时间数据
    time_datetime = []
    ...
    relative_error_once = mean_forecast_err1(training_NTLValues, alldataIndex, pred)
    print("relative error:", relative_error_once)
...
def fill_value(data):
    def _fill_value(data):
        ...
        df_pred = df.interpolate(limit_direction='both')
        pred = df_pred.loc[:, 'data'].values
        return pred
    ...
    for name in name_list:
        ...
        for i in range(len(datasets)):
            ...
            for j in range(cross_validation_count):
                result = fit_LinearInterpolation(eachPixel[0], eachPixel[1], eachPixel[2], eachPixel[3][j], eachPixel[4][j], key_list[i])
        ...
        # 结果写入文件
        outputFile = parent_dir + time_series_type + "\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_" + str(name) + "_more50_LinearInterpolation_accuracy_error_afterfit"
        f = open(outputFile, 'w')
        key_list = []
        if (len(datasets)):
