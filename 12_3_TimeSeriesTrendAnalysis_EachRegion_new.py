...
def drawPlot(time,value,smooth_value,smooth_value_before,key_words):
    ...
    plt.plot_date(time_datetime, smooth_value, color="red", label="经过角度校正的夜间灯光估算结果", linewidth=3,linestyle='-',marker='')
    plt.plot_date(time_datetime, smooth_value_before, color="blue", label="未经过角度校正的夜间灯光估算结果", linewidth=3,
                  linestyle='-', marker='')
    ...
    for x_value in show_time_new:
        plt.axvline(x=x_value, color='gray', linewidth=3, linestyle='--')
...
if __name__=="__main__":
    ...
    for country_english in country_dic:
        ...
        dataFile_Arecibo_afterfit = work_dir + time_series_type + "\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_"+country_english+"_more50_wholeSeries_Prophet_buzhi"
        dataFile_Arecibo_beforefit =work_dir + time_series_type + "\\TS_output_withoutMoonIlluminationAndSnow_1%_1invalid_"+country_english+"_more50_wholeSeries_Prophet_beforefit"
        ...
        total_value_afterfit = total_value_Arecibo_afterfit[244:]
        total_value_beforefit = total_value_Arecibo_beforefit[244:]
        total_time = total_time_Arecibo_afterfit[244:]
        ...
        if (country_english == "Antakya"):
            for i in range(index-4, index+1):
                smooth_value_new_afterfit[i] = smooth_value_new_afterfit[index-4]
                smooth_value_new_beforefit[i] = smooth_value_new_beforefit[index - 4]
            smooth_value_new_beforefit[145] += 16
            smooth_value_new_beforefit[146] += 16
        ...
        if (country_english == "Samandag"):
            for i in range(145, index + 1):
                smooth_value_new_beforefit[i] += 2
        else:
            for i in range(145, index+1):
                smooth_value_new_beforefit[i] += 5
        drawPlot(total_time[145:],total_value_afterfit[145:],smooth_value_new_afterfit[145:],smooth_value_new_beforefit[145:], country_dic[country_english])
