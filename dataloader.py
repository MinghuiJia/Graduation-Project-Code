...
def processPixelTimeSeries(line, time_list, normalized_ntl_list, vza_list, allData_Index):
    i_count = 0
    ...
    if (str(current_time)[-3:] != "001"):
        before_time = str(current_time)[:-3] + "001"
    ...
    if (str(end_time)[-3:] != "121"):
        after_time = str(end_time)[:-3] + "121"
...
def loadData(file_path):
    ...
    for row in range(len(lines)):
        if (row == 0):
            ...
            vza_list = []
            allData_Index = []
            key = processPixelTimeSeries(lines[row].strip(), time_list, normalized_ntl_list, vza_list, allData_Index)
            key_list.append(key)
...
if __name__ == "__main__":
    file_path = "D:\\Qt_Code\\test_data\\Angle_Normalization\\output_txt_normalization\\NTL_TimeSeries_Normalization.txt"
